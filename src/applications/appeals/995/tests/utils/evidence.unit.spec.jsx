    ...
    describe('edit mode', () => {
      it('should the correct title with spelled out index', () => {
        expect(getProviderDetailsTitle('edit', 1, 'nonVa')).to.contain(
          'the first location we should request your private provider or VA Vet Center records from',
        );

        expect(getProviderDetailsTitle('edit', 3, 'nonVa')).to.contain(
          'the third location we should request your private provider or VA Vet Center records from',
        );

        expect(getProviderDetailsTitle('edit', 20, 'nonVa')).to.contain(
          'the 20th location we should request your private provider or VA Vet Center records from',
        );

        expect(getProviderDetailsTitle('edit', 31, 'nonVa')).to.contain(
          'the 31st location we should request your private provider or VA Vet Center records from',
        );

        expect(getProviderDetailsTitle('edit', 63, 'nonVa')).to.contain(
          'the 63rd location we should request your private provider or VA Vet Center records from',
        );
      });
    });
  });

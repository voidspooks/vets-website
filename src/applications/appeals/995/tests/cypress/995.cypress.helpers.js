...
export const PRIVATE_EVIDENCE_PROMPT_RADIOS =
...
export const clickContinue = () =>
  cy
    .get('va-button[continue]', { selector: 'button' })
    .scrollIntoView()
    .click();
...
export const verifyNormalizedH3 = (expectedText, index = 0) =>
  cy
    .get('form h3')
    .eq(index)
    .should('be.visible')
    ...
    .should(text => {
      ...
      expect(normalized).to.eq(expectedText);
    });
...
export const clickArrayBuilderDeleteCardButton = index => {
  cy.get('va-card')
    .eq(index)
    .within(() => {
      cy.get('va-button-icon[data-action="remove"]').click();
    });
};

export const clickArrayBuilderDeleteModalYesButton = () => {
  cy.get('va-modal[modal-title="Remove this location?"]')
    .shadow()
    .find('va-button')
    .shadow()
    .find('button')
...
export const addVaTreatmentDate = (monthIndex, year) => {
  ...
  cy.get('va-text-input.input-year')
    .shadow()
    .find('input')
    .focus()
    .realType(year);
  ...
  cy.get('va-card')
    ...
    .within(() => {
      cy.get('h4')
        .should('exist')
        .and('be.visible')
        .and('have.text', locationDetails.name);

      const concatenatedAddress = `Mailing address:${
        locationDetails.addressLine1
      }${locationDetails.addressLine2 || ''}${locationDetails.city}, ${
        locationDetails.stateCode
      }${locationDetails.zip}United States`;

      verifyParagraph(concatenatedAddress, 0);

      if (conditionsCount > 1) {
        verifyParagraph(`Conditions: ${conditions}`, 1);
      } else {
        verifyParagraph(`Condition: ${conditions}`, 1);
      }

...
describe('Supplemental Claim keyboard only navigation', () => {
  it('navigates through a maximal form', () => {
    ...
    cy.get('@testData').then(data => {
      ...
      // eslint-disable-next-line cypress/no-unnecessary-waiting
      cy.wait(100);
      cy.tabToElement('[name="root_contestedIssues_0"]'); // Select headaches
      cy.realPress('Space');
      ...
      cy.typeInFocused(newIssue.issue);
      ...
      cy.tabToElement('[name="root_issues_Hypertension"]');
      cy.realPress('Space');
      tabToContinue();
      ...
      cy.tabToElement('[name="root_treatmentStartMonth"]');
      cy.realType(privateFromDate[1]); // month
      cy.tabToElement('[name="root_treatmentStartDay"]');
      cy.realType(privateFromDate[2]); // day
      cy.tabToElement('[name="root_treatmentStartYear"]');
      cy.realType(privateFromDate[0]); // year

      const privateToDate = facilityData.treatmentDateRange.to
        .split('-')
        .map(v => parseInt(v, 10).toString());
      cy.tabToElement('[name="root_treatmentEndMonth"]');
      cy.realType(privateToDate[1]); // month
      cy.tabToElement('[name="root_treatmentEndDay"]');
      cy.realType(privateToDate[2]); // day
      cy.tabToElement('[name="root_treatmentEndYear"]');

...
| <span id="dispensed-date">`dispensedDate`</span> | string/date | `"2024-10-31T04:00:00000Z"` | ⚠️ Despite the name, this is actually the pharmacy **release date** (VistA RDT[6]), not the VistA dispensed date (RDT[22]). Renamed from `releaseDateTime` in the MHV API. **Note:** On the **list endpoint**, if refills exist, this is overwritten with the most recent RF record's `releaseDateTime` (not the original RX's). On the **detail endpoint**, the parent RX record uses the original saved value. **⚠️ Oracle Health:** This field may be `null` for OH prescriptions even when the medication has been dispensed — OH uses `whenHandedOver` on refill records instead. Always check [sortedDispensedDate](#sorted-dispensed-date) and `rxRfRecords[].whenHandedOver` as fallbacks | Use the [sortedDispensedDate](#sorted-dispensed-date) field first, when available. Use this field as a fallback. For OH prescriptions, also check `rxRfRecords[].whenHandedOver`. ProcessList.jsx also pulls from `rxRfRecords[0].dispensedDate` as a source for the most recent dispensed date |
...
| <span id="sorted-dispensed-date">`sortedDispensedDate`</span> | string/date | `"2024-06-17"` | The [dispensedDate](#dispensed-date) field using the current sort order. **Note:** For Oracle Health prescriptions, this may be the only reliable dispense date on the parent object when `dispensedDate` is `null` | Used in the UI to represent the date a prescription was dispensed. Prefer this field over [dispensedDate](#dispensed-date) — use `dispensedDate` only as a fallback. Also used by `isUnfilledOhPrescription()` to determine if an OH prescription has ever been dispensed |
...
| `rxRfRecords` | array | Array of Prescription objects | An array of refill history data. **⚠️ Oracle Health:** OH refill records use `whenHandedOver` (date handed to patient) instead of `dispensedDate`. When checking if a prescription has ever been dispensed, check both `record.dispensedDate` and `record.whenHandedOver` | This field is used to display refill history data for a prescription. **NOTE:** The object at index `0` is the most recent refill |
...
**Cerner/Oracle Health Differences**: Cerner prescriptions undergo separate status mapping (e.g., Cerner `"OK"` → MHV `"pending"`), `isRefillable` is computed at DB ingest time (not display time), and `mhvStatus` defaults to `-6` when null. **Dispense date fields differ:** OH prescriptions may have `dispensedDate: null` on the parent object even when dispensed — `sortedDispensedDate` is computed by vets-api and is more reliable. OH refill records (`rxRfRecords`) use `whenHandedOver` instead of `dispensedDate` to indicate when the medication was given to the patient. The front-end `isUnfilledOhPrescription()` helper returns `false` when the backend marks the prescription as refillable via `isRefillable`; otherwise, it checks `sortedDispensedDate`, `dispensedDate`, and `rxRfRecords[].whenHandedOver` to determine if an OH prescription has ever been dispensed.

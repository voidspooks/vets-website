...
import { characterOfDischargeTypeLabels } from '../../constants/benefits';
...
const testConfig = createTestConfig(
  {
    ...
    pageHooks: {
      ...
      confirmation: () => {
        // Reload the confirmation page
        cy.reload();

        // Navigate back to the review page
        cy.get('[data-testid="back-link"]').click();

        // Expand all accordions to ensure answers are visible
        cy.get("va-button-icon[data-testid='expand-all-accordions']").click();

        // Check that the discharge answer is visible
        cy.get('@testData').then(data => {
          const displayText =
            characterOfDischargeTypeLabels[data.characterOfDischarge];

          cy.contains(displayText).should('be.visible');
        });

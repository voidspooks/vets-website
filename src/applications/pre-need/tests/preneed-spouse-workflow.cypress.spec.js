import testData from './schema/spouse-test.json';
import preneedHelpers from './utils/cypress-preneed-helpers';

describe('Pre-need form VA 40-10007 Spouse Workflow', () => {
  it('fills the form as a spouse of a living veteran', () => {
    preneedHelpers.interceptSetup();
    preneedHelpers.visitIntro();

    // Applicant Information - Spouse relationship
    ...
    cy.fill(
      '#root_application_claimant_name_maiden',
      testData.data.application.claimant.name.maiden,
    );
    cy.fill(
    ...
    // Should skip date of death page since sponsor is not deceased
    ...
    // No currently buried persons since answer was "2" (No)
    ...
    // Verify key information is displayed correctly
  ...
  it('fills the form as a spouse with additional currently buried persons', () => {
    preneedHelpers.interceptSetup();
    preneedHelpers.visitIntro();

    // Quick navigation to burial benefits
    cy.selectRadio('root_application_claimant_relationshipToVet', '2');
    preneedHelpers.clickContinue();
    ...
    cy.fill('#root_application_claimant_name_maiden', 'Williams');
    cy.fill('input[name="root_application_claimant_ssn"]', '222334444');
    cy.fillDate('root_application_claimant_dateOfBirth', '1980-05-15');
    preneedHelpers.clickContinue();

    cy.fillName('root_application_veteran_currentName', {
      first: 'John',
      middle: 'David',
      last: 'Johnson',
      suffix: 'Sr.',
    });
    cy.fill('input[name="root_application_veteran_ssn"]', '111223333');
    cy.fillDate('root_application_veteran_dateOfBirth', '1978-03-20');
    cy.fill('input[name="root_application_veteran_placeOfBirth"]', 'Chicago');
    preneedHelpers.clickContinue();
    ...
    cy.selectRadio('root_application_veteran_isDeceased', 'no');
    preneedHelpers.clickContinue();
    ...
    // Burial benefits with currently buried persons
    cy.fill(
      'input[name="root_application_claimant_desiredCemetery"]',
    ...
    cy.get('.autosuggest-item').should('exist');
    cy.get('body').click({ force: true });
    cy.selectRadio('root_application_hasCurrentlyBuried', '1');
    cy.axeCheck();
    preneedHelpers.clickContinue();

    // Add currently buried persons
    cy.fill(
    ...
    cy.fill(
      'input[name="root_application_currentlyBuriedPersons_1_name_first"]',
      'Elizabeth',
    );
    cy.fill(
      'input[name="root_application_currentlyBuriedPersons_1_name_last"]',
      'Johnson',
    );
    cy.axeCheck();
    preneedHelpers.clickContinue();
    ...
    cy.fill('input[name$="email"]', 'mary@example.com');
    cy.fill('input[name$="phoneNumber"]', '3125551234');
    preneedHelpers.clickContinue();

    cy.fillAddress('root_application_veteran_address', {
      street: '456 Oak Ave',
      city: 'Chicago',
      country: 'USA',
      state: 'IL',
    ...
    // Verify currently buried persons in review
    cy.get('.review').should('contain', 'Robert');
    cy.get('.review').should('contain', 'Elizabeth');

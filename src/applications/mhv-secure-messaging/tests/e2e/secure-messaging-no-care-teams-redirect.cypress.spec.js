...
import mockAllBlocked from './fixtures/recipientsResponse/all-blocked-recipients-response.json';
...
describe('Secure Messaging - No Care Teams Redirection', () => {
  ...
  describe('when all triage groups are blocked', () => {
    it('redirects to inbox when accessing Interstitial Page with all blocked care teams', () => {
      PatientInboxPage.loadInboxMessages(undefined, undefined, mockAllBlocked);

      cy.visit('/my-health/secure-messages/new-message');

      cy.location('pathname').should('include', Paths.INBOX);
      PatientInboxPage.validateBlockedTriageGroupAlert();

      cy.injectAxeThenAxeCheck(AXE_CONTEXT);
    });

    it('redirects to inbox when accessing Recent Care Teams with all blocked care teams', () => {
      PatientInboxPage.loadInboxMessages(undefined, undefined, mockAllBlocked);
    ...
    it('redirects to inbox when accessing Select Care Teams with all blocked care teams', () => {
      PatientInboxPage.loadInboxMessages(undefined, undefined, mockAllBlocked);

...
describe('Type 2 Failure Analytics', () => {
  it('should record a single event with the correct count for multiple alerts on the Claims list page', () => {
    ...
    trackClaimsPage.loadPage(claimsListWithFailures, claimDetailWithDates);
    ...
    cy.window()
      .its('dataLayer')
      .then(dl => {
        const type2Events = dl.filter(item => item.event === TYPE2_EVENT_NAME);
  ...
  it('should record a single event on Status page', () => {
    ...
    const trackClaimsPage = new TrackClaimsPageV2();
    trackClaimsPage.loadPage(claimsList, claimDetailWithDates);

...
function ClaimFileHeader({ claim, isOpen }) {
  const filesMeta = claim?.attributes?.claimStatusMeta?.files;
  const headerTitle = filesMeta?.headerTitle || 'Claim files';
  const headerDescription = isClaimOpen => {
    if (filesMeta?.description) return filesMeta.description;
  ...
  return (
    <div className="claim-file-header-container">
      <h2 className="tab-header vads-u-margin-y--0">{headerTitle}</h2>
      {filesMeta?.description ? (

  ...
  return createTestConfig(
    {
      ...
      pageHooks: {
        ...
        'supporting-evidence/private-medical-records-upload-enhancement': () => {
          ...
          /**
           * Target the va-select INSIDE the shadow DOM hierarchy.
           * `va-file-input-multiple` clones light DOM slot content
           * into each `va-file-input` instance. Interacting with the
           * cloned copy ensures `composedPath()` includes `VA-FILE-INPUT`,
           * which `getFileInputInstanceIndex` requires to find the
           * correct file index.
           */
          cy.get('va-file-input-multiple')
            .shadow()
            .find('va-select[name="attachmentId"]')
            .shadow()
            .find('select')
            .first()
            .should('not.be.disabled')
            .select('L049', { force: true });
        },
        ...
        'supporting-evidence/additional-evidence-enhancement': () => {
          ...
          /**
           * @see private-medical-records-upload-enhancement hook for
           * explanation of shadow DOM targeting strategy.
           */
          cy.get('va-file-input-multiple')
            .shadow()
            .find('va-select[name="docType"]')
            .shadow()
            .find('select')
            .first()
            .should('not.be.disabled')
            .select('L023', { force: true });
        },

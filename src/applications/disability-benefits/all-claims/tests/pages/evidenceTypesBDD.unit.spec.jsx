...
describe('evidenceTypes', () => {
  ...
  describe('Other Evidence Label', () => {
    ...
    // This test directly invokes updateUiSchema to validate the implementation within evidenceTypesBDD. It makes
    // assumptions based on the contract and purposes of updateUiSchema and does not attempt to simulate the entire form
    // lifecycle.
    it('updateUiSchema should return full SHA label when toggle changes from true to false', () => {
      const { updateUiSchema } = uiSchema['view:selectableEvidenceTypes'][
        'ui:options'
      ];

      // When toggle is true, should return short label
      const resultWhenTrue = updateUiSchema({
        disability526NewBddShaEnforcementWorkflowEnabled: true,
      });
      expect(resultWhenTrue['view:hasOtherEvidence']?.['ui:title']).to.equal(
        'Other evidence',
      );

      // When toggle is false, should return full SHA label
      const resultWhenFalse = updateUiSchema({
        disability526NewBddShaEnforcementWorkflowEnabled: false,
      });
      expect(resultWhenFalse['view:hasOtherEvidence']?.['ui:title']).to.equal(
        'Required Separation Health Assessment - Part A Self-Assessment or other documents like your DD Form 214, supporting (lay) statements, or other evidence',
      );
    });
  });

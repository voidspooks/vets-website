/**
 * E2E test for mock form prefill - Add Contact Info (form-only update).
 * Tests adding contact info field that only updates the form data,
 * not the VA profile.
 */
...
import userNoAddress from '../fixtures/mocks/user-no-address.json';
...
const testConfig = createTestConfig(
  {
      ...
      'contact-information': ({ afterHook }) => {
        afterHook(() => {
          cy.get('va-card-status')
            ...
            .shadow()
            .find('a')
            .contains('Add mailing address')
            .click();
          // add mailing address in the form only
          cy.selectVaSelect('root_countryCodeIso3', 'USA');
    ...
    setupPerTest: () => {
      cy.intercept('GET', '/v0/user', userNoAddress);
